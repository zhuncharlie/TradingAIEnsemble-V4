...
| Fitting on test data | Any calibration weight, contradiction-ontology threshold, router parameter, or meta-model parameter fit using final-test data | **Absolute rule, reiterated project-wide**: fitting happens on validation only; the final test set is touched exactly once, for a single evaluation pass, per adapter/method (see §5 below and `DATA_SPLIT_PROTOCOL.md` §5) |
