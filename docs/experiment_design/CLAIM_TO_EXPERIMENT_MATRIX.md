...
|---|---|---|---|---|---|
| **H1** — schema-level structural contradiction (§2.2 ontology, **binary `C` only** — see `EXPERIMENT_PROTOCOL.md` §2.2's post-adversarial-review tightening) is an out-of-sample, model-agnostic **incremental predictive signal** (not a causal claim — see guardrail above), independent of generic cross-adapter disagreement, for degraded forecast/policy quality (Q1/Q3 sign-mismatch, the sole primary `E`; Q4 policy quality is a secondary robustness check only, §2.3), beyond self-reported confidence | **Primary** | L1.3 | **Revised twice: once after `EXPERIMENT_PROTOCOL.md` §8's adversarial review (FAIL on the original design), once after a Session 3 fresh blind re-review (thread `019f7e01-0fe6-7092-ad68-1b1f1f7e3e94`, verdict REVISE, which found and fixed a residual acceptance-criterion contradiction between §2.3.2 and §2.4 — see `EXPERIMENT_PROTOCOL.md` §2.4's rewritten single acceptance rule). A second fresh-reviewer re-adjudication of this revision is still pending — treat this row as provisional until `H1_FRESH_REVIEW.md`'s final round completes.** Required evidence, now unambiguous: **the pooled nested-model test (§2.3.2, one row per `(ticker, as_of, horizon)` tuple per §2.3.1's unit-of-analysis fix, adapter-pair as a single random-effect/partial-pooling term by default per §2.3.1) must itself be significant — no per-stratum result may substitute for this.** `C`'s coefficient must remain significant after controlling for the precisely-defined generic disagreement-magnitude covariate (normalized Shannon entropy of directional votes, §2.3.1), a missingness/coverage indicator, and the adapter-pair random effect — significant at the pilot-stage-computed power-analysis threshold (§2.4); block-bootstrap CI excludes 0; leave-one-adapter-out AND leave-one-ticker-out robust on the *pooled* coefficient; minimum coverage met (§2.4.1); the per-stratum BH-FDR-corrected breakdown is reported only as secondary color on an already-pooled-significant result, never as an alternative path to "H1 supported" | Pooled H0 not rejected, OR the effect disappears once generic-disagreement/missingness/adapter-pair controls are added (i.e. H1 is really just "any disagreement predicts errors"), OR the pilot-computed minimum coverage (§2.4.1) is not met and no honestly-narrowed-scope alternative claim is substituted, OR leave-one-out shows the pooled effect is carried by a single adapter/ticker, OR (explicitly, per the fresh-review fix) the pooled test is null but some individual stratum is significant only before/without FDR correction — that is a null result, not a stratum-conditional pass | NOT YET RUN |
...
| **Fallback (c)**: the pooled H1 test (row 1's sole acceptance path) is significant, and the secondary per-stratum breakdown additionally shows the effect is markedly stronger in some regimes/horizons | Fallback | L1.3 + L1.4 | The pooled test (row 1) is significant on its own terms first; the per-stratum heterogeneity is then reported as regime-conditional *nuance* on an already-supported claim, not as what makes H1 supported | N/A — this row only applies once row 1's pooled bar is independently met; if the pooled test is null, see row 1's own falsification column (a null pooled test with an isolated significant stratum is a null result, not this fallback) — **rewritten after the Session 3 fresh blind review, thread `019f7e01-0fe6-7092-ad68-1b1f1f7e3e94`, found the prior wording of this row directly contradicted `EXPERIMENT_PROTOCOL.md` §2.3.2's pooled-primary rule** | NOT YET RUN |
