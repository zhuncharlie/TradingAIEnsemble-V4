...
| Best single adapter, selected on validation | Yes | Yes | No | Low | 1 selection | Adapter's own cost + selection overhead | Selection itself must not touch final test — see `DATA_SPLIT_PROTOCOL.md` §1 (interval-fitting rule); adapter eligibility itself is §5 |
